...
Blake2s::Blake2s(uint8_t length, string key) : outlen(length) {
	if (key.empty()) {
		blake2s_init(&(this->state), this->outlen);
	} else {
		blake2s_init_key(&(this->state), this->outlen, key.c_str(), (const uint8_t)key.length());
	}
}
Blake2s::~Blake2s() {}

Persistent<Function> Blake2s::constructor;

void Blake2s::Init(Isolate *isolate) {
	// Prepare template
	Local<FunctionTemplate> tpl = FunctionTemplate::New(isolate, New);
	tpl->SetClassName(String::NewFromUtf8(isolate, "Blake2s"));
...
void Blake2s::New(const v8::FunctionCallbackInfo<v8::Value>& args) {
	...
	if (args.IsConstructCall()) {
		...
		Blake2s* instance = new Blake2s(len, key);
...
void Blake2s::NewInstance(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = args.GetIsolate();
...
// ================ ]  PROTOTYPE  [ ================ //
void Blake2s::Update(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = args.GetIsolate();
	Blake2s* instance = ObjectWrap::Unwrap<Blake2s>(args.Holder());
	...
	if (node::Buffer::HasInstance(args[0])) { // if data is a buffer
		...
		// Perform update
		if (blake2s_update(&(instance->state), data, length) != 0) {
...
void Blake2s::Digest(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = args.GetIsolate();
	Blake2s* instance = ObjectWrap::Unwrap<Blake2s>(args.Holder());
	...
	// Perform digest
	if (blake2s_final(&(instance->state), hash, instance->outlen) != 0) {
		isolate->ThrowException(String::NewFromUtf8(isolate, "Internal blake2 error!"));
		return;
	}
