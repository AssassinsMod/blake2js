...
Blake2bp::Blake2bp(uint8_t length, string key) : outlen(length) {
	if (key.empty()) {
		blake2bp_init(&(this->state), this->outlen);
	} else {
		blake2bp_init_key(&(this->state), this->outlen, key.c_str(), (const uint8_t)key.length());
	}
}
Blake2bp::~Blake2bp() {}

Persistent<Function> Blake2bp::constructor;

void Blake2bp::Init(Isolate *isolate) {
	// Prepare template
	Local<FunctionTemplate> tpl = FunctionTemplate::New(isolate, New);
	tpl->SetClassName(String::NewFromUtf8(isolate, "Blake2bp"));
...
void Blake2bp::New(const v8::FunctionCallbackInfo<v8::Value>& args) {
	...
	if (args.IsConstructCall()) {
		...
		Blake2bp* instance = new Blake2bp(len, key);
...
void Blake2bp::NewInstance(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = args.GetIsolate();
...
// ================ ]  PROTOTYPE  [ ================ //
void Blake2bp::Update(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = args.GetIsolate();
	Blake2bp* instance = ObjectWrap::Unwrap<Blake2bp>(args.Holder());
	...
	if (node::Buffer::HasInstance(args[0])) { // if data is a buffer
		...
		// Perform update
		if (blake2bp_update(&(instance->state), data, length) != 0) {
...
void Blake2bp::Digest(const FunctionCallbackInfo<Value>& args) {
	Isolate* isolate = args.GetIsolate();
	Blake2bp* instance = ObjectWrap::Unwrap<Blake2bp>(args.Holder());
	...
	// Perform digest
	if (blake2bp_final(&(instance->state), hash, instance->outlen) != 0) {
		isolate->ThrowException(String::NewFromUtf8(isolate, "Internal blake2 error!"));
		return;
	}
